...
const validationParameters = (mainAmount, downPayment, numMonths, rate, finalPayment) => {
    ...
    if (mainAmount <= 0) errorsArray.push({ message: MAIN_AMOUNT_LIMITS });
    if (downPayment < 0) errorsArray.push({ message: DOWN_PAYMENT_LIMITS });
    if (mainAmount <= downPayment) errorsArray.push({ message: MAIN_AMOUNT_MORE_DOWN_PAYMENT_LIMITS });
    if (numMonths < 1) errorsArray.push({ message: MONTH_LIMITS });
    if (rate < 0) errorsArray.push({ message: RATE_LIMITS });
...
const monthlyPayment = (mainAmount, downPayment, numMonths, rate, finalPayment) => {
    ...
    if(preparedRate === 0) return { result: (checkedAmount/numMonths).toFixed(2), errors: [] };
    ...
    return { result: monthlyPaymentResult.toFixed(2), errors: [] };
};
